...
if (filters)
{
    filters = filters.replace("*", ""); // No need for wildcard for regex
}
...
async function asyncTest(name, expected, testFn)
{
    ...
    if (module_beforeFn) await module_beforeFn();
    if (testFn)
    {
        sub_testPass = 0;
        ...
        {
            console.log("\x1b[36m" + test_name + " \x1b[31m[FAILED]\x1b[0m (" + sub_testPass + " != " + expected + ")");
        }
    }
    if (module_afterFn) await module_afterFn();
}
...
{
    console.log("\x1b[36m" + test_name + " \x1b[31m[FAILED]\x1b[36m (" + expr + ")\x1b[0m ");
}
...
async function testEvent() {
    ...
    await asyncTest("sendEvent()", function() {
        var sendEventSemi = 0;
...
async function testFile() {
    module("File", null, null);

    await asyncTest("listUserFiles(\"\", true)", function() {
    ...
    await asyncTest("listUserFiles(null, null)", function() {
    ...
    await asyncTest("deleteUserFile()", function() {
    ...
    await asyncTest("deleteUserFiles()", function() {
